...
Shader "SineVFX/LivingParticles/LivingParticleAlphaBlendedURP"
{
	Properties
	{
		...
		_GroundCenter("Ground Center", Vector) = (0,0,0,0)
		_GroundInnerRadius("Ground Inner Radius", Float) = 2
		_GroundOuterRadius("Ground Outer Radius", Float) = 10
		_GroundFadePower("Ground Fade Power", Float) = 1

		_BaseAlpha("Base Alpha", Range(0, 1)) = 0.25
		_ReactionAlpha("Reaction Alpha", Range(0, 1)) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			struct VertexOutput
			{
				...
				float3 particleWorldPos : TEXCOORD6;
				UNITY_VERTEX_INPUT_INSTANCE_ID
			...
			float4 _GroundCenter;
			float _GroundInnerRadius;
			float _GroundOuterRadius;
			float _GroundFadePower;

			float _BaseAlpha;
			float _ReactionAlpha;
			...
			VertexOutput VertexFunction ( VertexInput v  )
			{
				...
				float3 vertexValue = float3(0, ResultMask53 * _OffsetPower, 0);
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float3 particlePos = IN.particleWorldPos.xyz;
				float3 groundCenter = _GroundCenter.xyz;

				float groundDist = distance(particlePos, groundCenter);

				float groundFade = saturate(
					(groundDist - _GroundInnerRadius) / max(0.0001, (_GroundOuterRadius - _GroundInnerRadius))
				);

				float groundMask = 1.0 - groundFade;
				groundMask = pow(groundMask, _GroundFadePower);

				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;

				float affectorMask = saturate(ResultMask53 * _FinalMaskMultiply);

				float3 baseColor = (1.0.xxx * IN.ase_color).rgb;
				float3 reactionColor = (staticSwitch81 * IN.ase_color * _FinalPower).rgb;

				float3 Color = lerp(baseColor, reactionColor, affectorMask);

				// 기본 영역은 옅게, affector 주변은 진하게
				float alphaByAffector = lerp(_BaseAlpha, _ReactionAlpha, affectorMask);

				float Alpha = clampResult74 * groundMask * alphaByAffector;
